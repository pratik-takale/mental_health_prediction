(() => {
  ...
  const API_BASE = "https://mental-health-prediction-y1wp.onrender.com";
  ...
  function bandFor(score) {
    if (score < 4) {
      return {
        ...
        context:
          "Your responses indicate elevated strain. Consider improving sleep, physical activity and reducing excessive screen time.",
      };
    }
    ...
    if (score < 7) {
      return {
        ...
        context:
          "Your overall routine looks fairly steady, with some opportunities to improve recovery and maintain healthy habits.",
      };
    }
    ...
    return {
      label: "Signal: Strong",
      context:
        "Your habits indicate a well-supported routine. Continue maintaining healthy sleep, activity and digital habits.",
    };
  }
      ...
      try {
        ...
        if (res.status === 422) {
          ...
          renderError(
            "Check your inputs",
            matched
              ? "The API rejected a few fields. Check the highlighted inputs."
        ...
        if (
          ...
          renderError(
            "Unexpected response",
            "The API responded, but the prediction score was missing or malformed."
          );
        ...
        renderError(
          ...
          `Couldn't connect to ${API_BASE}. Make sure FastAPI is running on port 2200.`
        );
      } finally {
        setSubmitting(false);
      }
